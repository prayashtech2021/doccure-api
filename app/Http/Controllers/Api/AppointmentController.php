...
class AppointmentController extends Controller
{
    ...
    public function getsignature($doctor_id)
    {
        ...
        if ($sign && !empty($sign->signature_image) && Storage::exists('images/signature/' . $sign->signature_image)) {
            $img = (config('filesystems.default') == 's3') ? Storage::temporaryUrl('images/signature/' . $sign->signature_image, now()->addMinutes(5)) : url('storage/images/signature/' . $sign->signature_image);
            $data = [
                'id' => $sign->id,
                'signature_image' => $img,
            ];
